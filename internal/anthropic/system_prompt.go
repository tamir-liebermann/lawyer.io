...
// It is intentionally stored as a const in Go source — the frontend must never
// send or modify it. See CLAUDE.md.
const SystemPromptBase = `אתה עוזר AI מקצועי המשרת משרד עורכי דין המתמחה בנדל"ן בישראל.
...
והקפד על פורמט: headers → נקודות → הערות.
`

// BuildSystemPrompt returns the full system prompt, optionally appending
// a mode-specific addendum and any real-estate data context gathered
// server-side for the current turn.
func BuildSystemPrompt(userType, realEstateContext string) string {
